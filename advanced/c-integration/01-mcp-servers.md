...
- [MCP vs Skills 选择指南](#mcp-vs-skills-选择指南)
...
近年来,Claude Code生态出现了一个明显的趋势:一些原本通过MCP服务器实现的功能,正在被封装为Skills。这种迁移带来了一些显著的优势。
...
**需求**: 查询远程PostgreSQL数据库

**选择**: MCP (PostgreSQL MCP Server)
- 原因: 需要外部数据库连接
- 优势: 标准化协议,性能好
- 配置: 需要配置数据库连接信息
...
| 任务 | MCP方式 | Skills方式 |
|------|---------|-----------|
| 浏览器自动化 | 需要开发和维护 | 使用现成Skill |
| 文档处理 | 需要编写代码 | 自然语言描述 |
| 工作流自动化 | 需要配置服务 | SKILL.md文件 |
...
浏览器操作截图:
- MCP方式: ~5000 tokens
- Skills方式: ~500 tokens (降低90%)

文档搜索:
- MCP方式: ~3000 tokens
- Skills方式: ~800 tokens (降低73%)
```
