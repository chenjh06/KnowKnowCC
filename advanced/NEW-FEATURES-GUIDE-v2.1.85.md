...
## 🆕 v2.1.89 新功能速览 (2026-04-01)

### ⏸️ `"defer"` 权限决策 ⭐⭐⭐⭐⭐

**重大更新**: PreToolUse hooks 现在可以返回 `{permissionDecision: "defer"}` 暂停 headless 会话，稍后通过 `-p --resume` 恢复。

```json
// PreToolUse hook 返回值
{
  "permissionDecision": "defer"  // 暂停等待人工决策
}
```
...
**应用场景**:
- Headless CI/CD 管道中需要人工审批的危险操作
- 自动化工作流中遇到模糊权限边界时的暂停点
- 与外部审批系统集成
...
# --mcp-config 服务器连接超时限制为 5 秒
# 不再阻塞在最慢的服务器上
```

**价值**:
- Headless/管道模式启动速度大幅提升
- 避免因单个 MCP 服务器响应慢而卡住整个会话
- 更可靠的 CI/CD 集成体验

### 🔄 Autocompact 循环修复 ⭐⭐⭐⭐

**问题**: 上下文压缩后，如果工具结果立即重新填满上下文窗口，Claude 会陷入"压缩→填满→压缩"的死循环，持续消耗 API 调用。

**修复**: 检测连续 3 次压缩后立即回填的情况，停止并给出可操作的错误提示。

**用户影响**:
- 避免长时间无效的 API 调用循环
- 明确告知用户需要 `/clear` 或减少上下文使用
- 节省 token 消耗
...
| 类别 | 改进 | 说明 |
|------|------|------|
| **Edit 增强** | Bash 查看文件可直接编辑 | `sed -n`/`cat` 查看的文件无需先 `Read` |
| **Hook 输出** | 超大输出存磁盘 | >50K 字符保存到文件，注入路径+预览 |
| **Bash 智能警告** | 格式化器冲突检测 | 格式化器修改已读文件时发出警告 |
| **@ 补全优化** | 排名改进 | 源代码文件排名高于相似 MCP 资源 |
| **LSP 健壮性** | 自动重启 | 崩溃后下次请求自动恢复 |

### 🔧 关键修复

| 修复 | 说明 |
|------|------|
| **`-p --resume` 挂起** | deferred 输入超 64KB 或无 deferred 标记时修复 |
| **hooks `if` 条件** | 修复不匹配复合命令(`ls && git push`)和环境变量前缀(`FOO=bar cmd`) |
| **LSP 僵尸状态** | 服务器崩溃后不再永久失效，自动重启 |
| **macOS 深度链接** | `claude-cli://` 链接现在能正常打开 |
| **MCP 错误截断** | 多元素错误内容不再只显示第一个 |
| **SDK 图片消息** | 发送图片时 skill 提醒不再丢失 |
| **Hook 绝对路径** | PreToolUse/PostToolUse 接收 Write/Edit/Read 的绝对路径 |
| **天城文截断** | Devanagari 等组合标记文本不再被截断 |
| **iTerm2/tmux 抖动** | 流式输出时 UI 不再周期性抖动 |
| **PowerShell 加固** | PS 5.1 含双引号+空白参数触发确认 |
| **`cleanupPeriodDays: 0`** | 不再静默禁用 transcript 持久化，报验证错误 |

### 📝 行为变更

| 变更 | 说明 |
|------|------|
| Hook 输出 >50K | 保存到磁盘，注入文件路径+预览 |
| `Edit` 扩展 | 可编辑通过 Bash (sed/cat) 查看的文件 |
| TaskCreated Hook | 记录其阻塞行为文档 |
| Ctrl+B 后台 | 保留任务通知 |
| `/buddy` | 愚人节彩蛋：孵化一只陪你编程的小生物 |
